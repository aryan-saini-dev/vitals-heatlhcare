...
export const Route = createFileRoute("/dashboard/patients/add")({
...
export default function AddPatient() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      void navigate({ to: "/dashboard/patients" });
    }
    setLoading(false);
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-5 relative z-10">
          ...
          <div className="pt-4 flex justify-end">
            <button
              type="submit"
              disabled={loading}
              className="w-full md:w-auto inline-flex items-center justify-center gap-2 h-10 px-6 bg-gradient-primary text-primary-foreground font-bold rounded-xl shadow-glow transition-all hover:scale-[1.02] disabled:opacity-50 disabled:scale-100"
            >
