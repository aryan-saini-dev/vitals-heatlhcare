...
export default function Calls() {
  ...
  const isExact = location.pathname === "/dashboard/calls" || location.pathname === "/dashboard/calls/";
    ...
    <div className="space-y-8 animate-fade-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 border-b border-border/40 pb-5">
        ...
        <button
          ...
          className="self-start shrink-0 inline-flex items-center gap-2 rounded-full border border-border bg-background px-4 py-2.5 text-xs font-semibold text-foreground hover:bg-secondary transition-all"
        >
                    ...
                    <tr key={call.id} className="hover:bg-secondary/20 transition-colors group">
                        ...
                        {call.vitals_data?.DoctorDecision ? (
                          ...
                          <div className="flex items-center justify-center gap-1">
                            <button
                              ...
                              className="px-2 py-1 text-xs font-bold border border-success/30 rounded-lg bg-success/10 text-success hover:bg-success hover:text-success-foreground transition-all"
                            >
                              ✓
                            </button>
                            <button
                              ...
                              className="px-2 py-1 text-xs font-bold border border-destructive/30 rounded-lg bg-destructive/10 text-destructive hover:bg-destructive hover:text-destructive-foreground transition-all"
                            >
                              ✕
                            </button>
                          </div>
                      ...
                      <td className="p-4 text-right">
                        <Link
                          ...
                          params={{ id: call.id }}
                          className="inline-flex items-center gap-1 px-3 py-1.5 bg-secondary text-foreground hover:bg-primary hover:text-primary-foreground rounded-lg font-bold text-xs border border-border/80 hover:border-transparent transition-all whitespace-nowrap"
                        >
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-border/40">
                      ...
                      {call.vitals_data?.DoctorDecision ? (
                        ...
                        <>
                          <button
                            type="button"
                            onClick={() => void setDecision(call, "approved")}
                            className="px-2.5 py-1.5 text-xs font-bold border border-success/30 rounded-lg bg-success/10 text-success"
                          >
                            Approve
                          </button>
                          <button
                            type="button"
                            onClick={() => void setDecision(call, "denied")}
                            className="px-2.5 py-1.5 text-xs font-bold border border-destructive/30 rounded-lg bg-destructive/10 text-destructive"
                          >
                            Deny
                          </button>
                        </>
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        disabled={!call.vitals_data?.ReportData}
                        onClick={() => void downloadReport(call.id)}
                        className="p-1.5 border border-border/80 rounded-lg bg-secondary disabled:opacity-40"
                      >
                      ...
                      <Link
                        to="/dashboard/calls/$id"
                        params={{ id: call.id }}
                        className="inline-flex items-center gap-1 px-3 py-1.5 bg-secondary text-foreground rounded-lg font-bold text-xs border border-border/80"
                      >
