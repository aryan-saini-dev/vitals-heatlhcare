    ...
    <div className="space-y-8 animate-fade-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 border-b border-border/40 pb-5">
        ...
        <button
          ...
          className="self-start shrink-0 inline-flex items-center gap-2 rounded-full bg-gradient-primary px-5 py-2.5 text-xs font-bold text-primary-foreground shadow-glow transition-all hover:scale-[1.03] disabled:opacity-50 disabled:scale-100 disabled:cursor-not-allowed cursor-pointer"
        >
      ...
      {!loading && stats.patients === 0 && stats.agents === 0 ? (
        <section className="bg-card border border-dashed border-border/80 rounded-3xl p-12 text-center max-w-2xl mx-auto flex flex-col items-center shadow-soft">
          ...
          <button
            onClick={handleSeedData}
            disabled={seeding}
            className="inline-flex items-center gap-2 rounded-full border border-primary/30 bg-background/50 px-6 py-2.5 text-xs font-semibold text-primary backdrop-blur transition-all hover:bg-primary hover:text-primary-foreground cursor-pointer"
          >
            Get Started
